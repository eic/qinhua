...
int Fun4All_G4_Momentum(const int nEvents = 1000, const char *outfile = NULL)
{
  ...
  gen = new PHG4ParticleGenerator("PGENERATOR2");
  gen->set_name("e-");
  gen->set_vtx(0, 0, 0);
  gen->set_eta_range(-0.5, +0.5);
  gen->set_mom_range(0.5, 0.5); // GeV/c
  gen->set_phi_range(0., 270. / 180. * TMath::Pi());  // 0-90 deg
  se->registerSubsystem(gen);

  gen = new PHG4ParticleGenerator("PGENERATOR3");
  gen->set_name("pi-");
  gen->set_vtx(0, 0, 0);
  gen->set_eta_range(-0.5, +0.5);
  gen->set_mom_range(0.5, 0.5); // GeV/c
  gen->set_phi_range(90., 180. / 180. * TMath::Pi());  // 0-90 deg
  se->registerSubsystem(gen);
...
{
  Fun4AllServer *se = Fun4AllServer::instance();
  PHG4TrackFastSim *ftrk = (PHG4TrackFastSim *) se->getSubsysReco("PHG4TrackFastSim");
  ftrk->DisplayEvent();
}
