...
HitNtuple::HitNtuple(const std::string &name, const std::string &filename)
  : SubsysReco(name)
  , nblocks(0)
  , hm(nullptr)
  , _filename(filename)
...
int HitNtuple::Init(PHCompositeNode *)
{
  hm = new Fun4AllHistoManager(Name());
  outfile = new TFile(_filename.c_str(), "RECREATE");
  ntup = new TNtuple("hitntup", "Hits", "detid:row:slat:x0:y0:z0:x1:y1:z1:edep");
  //  ntup->SetDirectory(0);
...
int HitNtuple::process_event(PHCompositeNode *topNode)
{
...
int HitNtuple::End(PHCompositeNode *topNode)
{
  outfile->cd();
  ntup->Write();
  outfile->Write();
...
void HitNtuple::AddNode(const std::string &name, const int detid)
{
